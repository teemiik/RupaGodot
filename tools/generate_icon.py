...
# Адаптивная иконка: мотив в safe-zone; круглая маска лаунчера ≈ 72/108
# поля, поэтому отверстие должно быть заметно меньше, иначе оно съедает
# весь значок (кольцо градиента почти пропадает).
GC = np.array([100.5, 99.5])   # центр группы «отверстие+шар» в 200-пространстве
GROUP_EXT = 97.0                # макс. габарит группы
ADAP_TARGET = 76.0              # ~54% диаметра круглой маски (было 120 → ~85%)
